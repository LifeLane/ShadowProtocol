    ...
    return (
        <AnimatedSection id="roadmap" className="bg-circuit-pattern">
            <div className="z-10 w-full">
                <h2 className="text-5xl md:text-7xl font-bold text-center mb-16 text-primary glow">Roadmap – The Mind's Growth</h2>
                    ...
                    {roadmapPhases.map((item, index) => (
                        ...
                        >
                            <div className={`w-full pl-12 md:w-1/2 md:pl-0 ${index % 2 === 0 ? 'md:pr-8 md:text-right' : 'md:pl-8 md:text-left md:order-3'}`}>
                                <h3 className="text-3xl font-bold text-accent">{item.phase}</h3>
                                <p className="text-foreground/70 mt-2 text-xl">{item.description}</p>
                                <p className={`mt-2 font-bold text-xl ${item.status === 'Complete' ? 'text-primary' : 'text-accent'}`}>&gt; {item.status}</p>
                            </div>
