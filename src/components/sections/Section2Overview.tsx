...
const Section2Overview = () => {
    ...
    return (
        <AnimatedSection id="token-overview" className="bg-pulse-grid-pattern">
            <Terminal title="SHADOW_SPEC.md" className="max-w-6xl z-10">
                <motion.div
...
Utility:   Fuel for On-Chain Mind`}
                ...
                >
                    {features.map((item, index) => (
                        <motion.div
                            key={index}
                            className="p-4 border border-primary/20 rounded-md bg-black/20 transition-all cursor-pointer group text-left card-hover-glow"
                            variants={{
