...
const Terminal = ({ children, title, className = '' }: TerminalProps) => {
  return (
    ...
    >
      <div className="bg-gray-900/50 rounded-t-lg px-4 py-2 border-b border-primary/20 flex items-center gap-4">
        <div className="flex space-x-2">
          <div className="w-3 h-3 rounded-full bg-red-500"></div>
          <div className="w-3 h-3 rounded-full bg-yellow-500"></div>
          <div className="w-3 h-3 rounded-full bg-green-500"></div>
        </div>
        {title && <p className="text-center text-sm text-muted-foreground flex-grow truncate">{title}</p>}
      </div>
